...
const Product = ({ products }) => {
  ...
  const starFilterClick = (star) => {
    ...
    const filteredProducts = products.filter(product => {
      const productRating = product.rating?.value || 0;
      return Math.floor(productRating) === star;
    });
    setPd(filteredProducts);
    setCurrentImages([]);
    setFilter('rating');
    setPageCount(0);
    setImagesOffset(0);
  ...
  useEffect(() => {
    if (filter === 'rating') {
      setPd(pd.sort((a, b) => (b.rating?.value || 0) - (a.rating?.value || 0)));
    } else if (filter === 'price-low-high') {
      setPd(pd.sort((a, b) => (Number(a.priceRange?.minVariantPrice?.amount) || 0) - (Number(b.priceRange?.minVariantPrice?.amount) || 0)));
    } else if (filter === 'price-high-low') {
      setPd(pd.sort((a, b) => (Number(b.priceRange?.minVariantPrice?.amount) || 0) - (Number(a.priceRange?.minVariantPrice?.amount) || 0)));
  ...
  return (
    <>
      <Head>
        <title>Next.js Shopify | Our Products</title>
          ...
          <div className='filters hidden md:flex flex-col basis-full md:basis-1/6 min-w-[120px]'>
            ...
            {(starRating !== '') && (
              <>
                ...
                {(starRating) && <p className='cursor-pointer px-2 py-1 inline-block bg-red-500 uppercase font-bold text-white mt-4 w-30 text-center' onClick={clearAllFilters}>Clear Filter</p>}
              </>
            )}
            ...
            <div className='ratings pt-4 flex flex-col gap-4'>
              {[5, 4, 3, 2, 1].map(star => (
                <div key={star} className='rating inline-flex gap-1 cursor-pointer' onClick={() => starFilterClick(star)}>
